...
bool Filter(NSURLRequest *request){
  _Log(@"ssssss:%@",request.URL.absoluteString);
  NSString* url = request.URL.absoluteString;
  if ([url containsString:@"client-api.itunes.apple.com/WebObjects/MZStorePlatform.woa/wa/lookup?caller="]) {
    return true;
  }
  /*
  if ([url containsString:@"mzstatic.com/image/thumb/"]) {
    return true;
  }
  if ([url containsString:@"mzstatic.com/us/"]) {
    return true;
  }*/
  return false;
}
